...
namespace Sol_PuntoVenta.Presentacion
{
    public partial class Frm_Departamentos : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgv_Listado.Columns[1].HeaderText = "CÓDIGO DE";
        ...
        private void Mostrar(string Ctexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Departamentos.Mostrar_de(Ctexto);
                this.Formato();
        ...
        {
            if (String.IsNullOrEmpty(Convert.ToString(Dgv_Listado.CurrentRow.Cells["codigo_de"].Value)))
            {
        ...
        private void Estado_Limpiar(bool Lestado)
        {
            ...
            Txt_descripcion_de.ReadOnly = !Lestado;
        }
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    E_Departamentos oPro = new E_Departamentos();
                    string Cduplica, Rpta = "";
                    oPro.Codigo_de = Codigo_de;
                    oPro.Descripcion_de = Txt_descripcion_de.Text.Trim();                    
                    // Verificamos si la información que se intenta guardar ya existe en la tabla
                    Cduplica = N_Departamentos.Verifica_duplicado_de(this.EstadoGuarda, oPro.Codigo_de, oPro.Descripcion_de);
                    if (Cduplica == "")
                    {
                        Rpta = N_Departamentos.Guardar_de(this.EstadoGuarda, oPro);
                        if (Rpta.Equals("OK"))
                        {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in Dgv_Listado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = N_Departamentos.Eliminar_de(Codigo);
                            if (Rpta.Equals("OK"))
                            {
        ...
        {
            Frm_Rpt_Departamentos ORpt_de = new Frm_Rpt_Departamentos();
            ORpt_de.Txt_p1.Text = Txt_buscar.Text;
            ORpt_de.ShowDialog();
        }
    }
}
